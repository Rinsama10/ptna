...
from ptna_ptna import Emotion,Ptna
...
def talk():
    ...
    change_looks()
def writeLog():
    now ='ptnaSystem Dialogue Log: '+datetime.now().strftime('%y-%m-%d %H:%m:*%S'+'\n')
    log.insert(0,now)
...
def run():
    ...
    #イメージを用意
    ptyna_images.append(tk.PhotoImage(file ='yumekawa_usagi.gif'))
    ...
    sb2.grid(row = 0,column=1,sticky=tk.EW)
